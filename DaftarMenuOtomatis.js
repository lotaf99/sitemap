...
function showHeaderOption(){
    ...
    s += "<table>";
    s += "<tr>";
    s += "<td style='text-align:right'>Sort by: ";
    s += "<td><select onchange='sortBy(this.value.substr(1), this.value.substr(0,1))'>";
    s += "<option value='0published'/>Date publish";
    s += "<option value='1title'/>Title";
    s += "</select>";
    s += "<tr>";
    s += "<td style='text-align:right'>Sort by label: ";
    s += "<td><select onchange='showPostsWLabel(this.value)' id='cl_labels'>";
    s += "<option value='*'/>All label";
    ...
    s += "</select>";
    s += "<tr>";
    s += "<td><td><a href='javascript:showPostsWLabel(\"*\");'>See all content</a>";
    s += "</table>";
...
function showHideSummary(obj){
    ...
    if(p.innerHTML == ""){
        ...
        obj.innerHTML = "▼";
        obj.title = "hide summary";
    } else {
        p.innerHTML = "";
        obj.innerHTML = "►";
        obj.title = "open summary";
    }
}
...
function titleCode(entry){
    var s = "<span title='open summary' onclick='showHideSummary(this)' style='cursor:pointer'>►</span> ";
